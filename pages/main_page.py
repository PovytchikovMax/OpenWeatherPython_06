...
class MainPage(BasePage):
    ...
    def check_subscribe_for_free_link_is_visible(self):
        subscribe_for_free_link = self.element_is_visible(self.locators.SUBSCRIBE_FOR_FREE_LINK)
        assert subscribe_for_free_link, "The Subscribe for free link is not visible"

    def check_subscribe_for_free_link_is_clickable(self):
        self.driver.find_element(*self.locators.COOKIES).click()
        subscribe_for_free_link = self.driver.find_element(*self.locators.SUBSCRIBE_FOR_FREE_LINK)
        assert subscribe_for_free_link.is_enabled(), "The Subscribe for free link is not clickable"
    ...
    def check_faq_link_is_visible(self):
    ...
    def check_terms_and_conditions_of_sale_link_is_clickable(self):
        self.driver.find_element(*self.locators.COOKIES).click()
        terms_and_conditions_of_sale_link = self.driver.find_element(*self.locators.TERMS_AND_CONDITIONS_OF_SALE_LINK)
        assert terms_and_conditions_of_sale_link.is_enabled(), "The Terms and conditions of sale link is not clickable"

    def check_privacy_policy_link_visibility(self):
    ...
    def check_content_of_company_module_text(self, text):
        element_text = self.driver.find_element(*self.locators.COMPANY_TEXT).text
        assert element_text == text, \
            f"Actual content '{element_text}' of the Company module text does not match expected '{text}'"
    ...
    def check_facebook_link_clickaility(self):
        facebook_link = self.driver.find_element(*self.locators.FACEBOOK_LINK)
    ...
    def check_twitter_link_clickaility(self):
        twitter_link = self.driver.find_element(*self.locators.TWITTER_LINK)
